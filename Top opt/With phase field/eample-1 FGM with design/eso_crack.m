...
nelx=60; nely=120; X0=0.0512; 
%% initialization for topology optimization
x = zeros(nely,nelx); % 1-inclusion, 0-matrix
x(1:nely,1:nelx) = X0;
% vol = mean(x(:));
maxiter=250;
elsize = 100./nely; % millimeter (mm)
l = 2*elsize; % mm
rmin=2*elsize;
loadN=100;

passive = zeros(nely,nelx);
passive(59:61, 1:15)=1;
%passive(nely/2-2*l/elsize:nely/2+1+2*l/elsize,1:nelx/4+2*l/elsize) = 1;
x(passive==1) = 0.001;
pore=find(passive==1);


passive2=zeros(nely, nelx); 
passive2(55:65, 15:40)=1; 
x(passive2==1) = 1;
...
%     dc = sqrt(abs(dc));
    ...
    [x,olddc] = optimizer_simp(x,dc,vol,passive);
    %% MMA
