...
nelx=200; nely=100; volfrac=0.5; rmin=4; ft=2; penal=3;
...
E1=210; % Define the Youngs modulus of material 1
E2=E1*10; % define the Youngs modulus of Material2
VF_alp=4; % Power for the volume fraction
...
while change > 0.01
  ...
  c = sum(sum((xPhys.^penal.*E0+(1-xPhys.^penal).*Emin).*ce));
  dc = (E0-Emin).*-penal.*xPhys.^(penal-1).*ce;
  dv = ones(nely,nelx);  
  ...
  x = xnew;
  %% PRINT RESULTS
  fprintf(' It.:%5i Obj.:%11.8f Vol.:%7.3f ch.:%7.6f\n',loop,c, ...
    mean(xPhys(:)),change);
  %% PLOT DENSITIES
